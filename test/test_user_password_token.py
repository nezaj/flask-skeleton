...
class TestUserPasswordToken:
    ...
    def test_invalid_tokens(self, user, db):
        ...
        # Valid token
        valid_token = UserPasswordToken(user=user, used=False).save(db.session)
    ...
    def test_get_or_create_token(self, user, db):
        ...
        token = UserPasswordToken.get_or_create_token(db.session, user.id)
        assert user_tokens_query.all() == [token]

        # The same token is returned while it is still valid.
        assert UserPasswordToken.get_or_create_token(db.session, user.id) == token
